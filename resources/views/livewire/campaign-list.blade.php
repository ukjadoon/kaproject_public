                            ...
                            <div class="min-w-0 flex-1 px-4 md:grid md:grid-cols-2 md:gap-4">
                                <div>
                                    ...
                                    <div class="mt-2 flex items-center text-sm leading-5 text-gray-500">
                                        ...
                                        <div>
                                            <span>{{ $this->getClientsFormCampaign($campaign['id']) }}</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="hidden md:block">
                                    <div>
                                        @foreach($this->getMunicipalitiesForCampaign($campaign['id']) as $municipality)
                                        <div class="text-sm leading-5 font-medium text-indigo-600 truncate">
                                            <a class="hover:underline" href="{{ route('campaign-landing', ['code' => $campaign['code'], 'slug' => $municipality['slug']]) }}" target="_blank">{{ $municipality['name'] }}</a>
                                        </div>
                                        @endforeach
                                        <div class="mt-2 flex items-center text-sm leading-5 text-gray-500">
